...
import (
	...
	"study/blog-system/handlers"
	"study/blog-system/middleware"
)
...
func main() {
	...
	// 需要认证的路由组 使用权限中间件校验是否登录
	auth := r.Group("/api").Use(middleware.AuthMiddleware())
	{
		// 文章相关接口
		...
		auth.GET("/posts/:id/comments", handlers.GetComments)
	}
